...
class Road:
    ...
    def __init__(self, road_id, pixmap, x=0, y=0):
        ...
        self.x = x
        self.y = y
        self.locked = False  # 是否锁定（锁定后不可移动/删除）
...
class DraggablePixmapItem(QGraphicsPixmapItem):
    """可拖动的图片项（用于摄像头、车辆初始位置和道路）"""
    def __init__(self, pixmap, data_object, editor, object_type="item"):
        super().__init__(pixmap)
        self.data_object = data_object  # Camera、Vehicle 或 Road 对象
        self.editor = editor
...
class VehicleEditor(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("车辆轨迹编辑器 V2")
    ...
    def add_vehicle_to_scene(self):
        ...
        self.update_status(f"添加 {vehicle_name} 车辆 #{vehicle_id} 到场景")
        self.redraw_scene()
    ...
    def redraw_scene(self):
        ...
        # 调整视图
        if self.roads:
            self.scene.setSceneRect(0, 0, 2000, self.roads[-1].y + self.roads[-1].pixmap.height() + 100)
    ...
    def on_road_selected(self, item):
        """道路列表点击"""
        # v2 当前道路列表仅用于展示；被点击时给出状态提示，避免 UI 绑定缺失导致崩溃
        idx = self.road_list.row(item)
        if 0 <= idx < len(self.roads):
            self.update_status(f"选中道路 #{self.roads[idx].id}")
        else:
            self.update_status("选中道路素材")
    ...
    def fit_view_to_roads(self):
        ...
        # 计算所有道路的总高度
        last_road = self.roads[-1]
        total_height = last_road.y + last_road.pixmap.height()
        
        # 计算总宽度（左侧留白 + 道路宽度 + 右侧留白）
        if self.roads:
            total_width = self.left_margin + self.roads[0].pixmap.width() + 200
        else:
            total_width = 2000
        
        # 更新场景矩形
        self.scene.setSceneRect(0, 0, total_width, total_height + 100)
        
        # 自动调整视图以适应场景
        self.view.fitInView(self.scene.sceneRect(), Qt.AspectRatioMode.KeepAspectRatio)
        
        self.update_status(f"视图已自适应调整 (宽:{total_width} 高:{total_height})")
