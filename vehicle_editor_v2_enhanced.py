...
from PyQt6.QtWidgets import (
    ...
    QColorDialog, QInputDialog, QGraphicsRectItem
)
...
class VehicleEditor(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("车辆轨迹编辑器 V2 - 增强版")
        ...
        # 布局参数
        self.left_margin = 200  # 左侧留白区域宽度（可设置）
    ...
    def create_left_panel(self):
        ...
        # 背景颜色设置
        btn_bg_color = QPushButton("设置背景颜色")
        btn_bg_color.clicked.connect(self.choose_background_color)
        scene_layout.addWidget(btn_bg_color)
    ...
    def create_center_panel(self):
        ...
        self.view.setBackgroundBrush(QBrush(self.background_color))
    ...
    def on_margin_changed(self, value):
        ...
        self.update_status(f"左侧留白设为 {value}px")
        self.redraw_scene()
    ...
    def clear_trajectory(self):
        ...
        if len(self.selected_vehicle.trajectory) > 0:
            ...
            self.selected_vehicle.trajectory = []
    ...
    def redraw_scene(self):
        ...
        if self.left_margin > 0:
            margin_rect = QGraphicsRectItem(0, 0, self.left_margin, 2000)
            margin_rect.setBrush(QBrush(QColor(50, 50, 50, 100)))
            margin_rect.setPen(QPen(Qt.PenStyle.NoPen))
        ...
        # 调整视图
        if self.roads:
            self.scene.setSceneRect(0, 0, 2200, self.roads[-1].y + self.roads[-1].pixmap.height() + 100)
